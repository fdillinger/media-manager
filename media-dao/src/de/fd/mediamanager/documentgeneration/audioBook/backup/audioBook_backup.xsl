...
<!-- ******************************************** XSL DYNAMIC PART ***************************************** -->
 ...
 <w:p wsp:rsidR="000F62D6" wsp:rsidRDefault="00052899"> 
	...
	<xsl:for-each select="list/author">
		<w:r>
			<w:t>
				Last name : <xsl:value-of select="lastname"/><xsl:text> </xsl:text>First name : <xsl:value-of select="firstname"/>
				<w:br w:type="text-wrapping"/>	
				<xsl:for-each select="books/book">
					title :<xsl:text> </xsl:text><xsl:value-of select="title"/><xsl:text> </xsl:text><w:br w:type="text-wrapping"/>
				</xsl:for-each>		
				<w:br w:type="text-wrapping"/>
			</w:t>	
			<w:br w:type="text-wrapping"/>	
